...
public class NomalConversation extends Conversation {
    ...
    public String getAvatarUrl() {
        String avatarUrl = FriendshipInfo.getInstance().getProfile(identify).getAvatarUrl();
        if (avatarUrl != null) {
            return avatarUrl;
        }
        return null;
    ...
    public void navToDetailAndUrl(Context context,String faceUrl) {
        ChatActivity.navToChat(context, identify, type,faceUrl);
    }
    ...
    public String getLastMessageSummary() {
        ...
        if (ext.hasDraft()) {
            ...
            if (lastMessage == null || lastMessage.getMessage().timestamp() < ext.getDraft().getTimestamp()) {
                return MyApplicaiton.getContext().getString(R.string.conversation_draft) + textMessage.getSummary();
            } else {
